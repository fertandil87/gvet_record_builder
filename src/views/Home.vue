    ...
    <el-container>
      <el-main :style="cssProps">
        <button @click="saveData">submit</button>
          <button @click="loadData">Load Json</button>
		      <button @click="showJson">Toggle Json</button>
        <div style="display:flex">
          <div class="wrapper--forms" style="flex:1">
          ...
          <button @click="hasGroup = true">Add group</button>
          <div v-show="hasGroup">
            <div class="wrapper--forms" style="flex:1">
            <!-- Show empty state if empty -->
            

            <div v-if="forms1.length == 0" class="emptyState">
              <span class="emptyState__emoji">╭( ✖_✖ )╮</span>
          ...
          <div class="wrapper--snippet" v-show="isVisible">
            <pre>{{ forms1 }}</pre>
            <Output></Output>
          </div>
      </el-main>
...
<script>
  ...
  export default {
    ...
    mounted(){
      // this.forms1 = this.forms
      Bus.$on('loadData', function(response) {
        this.forms = null      
      })
